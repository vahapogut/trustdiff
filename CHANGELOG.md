...
- TD013 `exotic-source` (block): a lockfile entry resolved from git, a tarball, a local path or an origin the file does not state. A private registry serving the usual tarball layout is not exotic.
