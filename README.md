...
```yaml
- uses: vahapogut/trustdiff@c3a0824e2c99f01ecdc0f8b084bad484fee1e37f # v0.5.1
  with:
    fail-on: block
    format: sarif
```
...
The sha is a commit that comes after that release's tag, normally the
`chore(action): pin <tag> and checksums` one, not the tag. The table
the action verifies a download against can only be written once the archives
exist, so it is filled in afterwards, and the action at the tag itself still
defaults to the release before it. Pinning at that commit is what makes
`version` unnecessary: at that sha the default already is that release. v0.5.1
is also the first tag whose manifest a runner can load: an earlier tag fails
before a single step of the action runs. Pass `version:` only to run a different
one, which takes the cosign route described below.
