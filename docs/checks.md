...
| `standing` | what the level turned on: `young`, `low-usage`, `established`, `age-unknown` or `usage-unknown` |

**Example.** The 2017 package. npm removed the malicious versions, but the name is still registered and still collects traffic, so the run reads it as one that has been around for years and reports the match at `warn` rather than at `block`. Captured live on 2026-09-10:

```
npm:crossenv@6.1.1  WARN
  warn
    TD008 typosquat-suspect: "crossenv" resembles the popular npm package "cross-env"
      ...
      popular "cross-env" as a similarly named package. The level is lowered to warn because the
      package is one a project has been living with: it has been on the registry since
      2017-07-19T04:21:00Z and has 934 weekly downloads, at or above the low-usage threshold of 500
```

That is what this level rule costs, in the one direction where it costs something: nine years of scanner traffic is enough to read as established. It is worth saying what it does not cost. A squat is caught while it is a squat, in the days or weeks before anyone notices it, and there it is young, barely installed, or both. This one was noticed in 2017.
