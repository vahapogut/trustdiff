...
| 1.14 | Bare manifest refs | `check package.json`, `check pyproject.toml`, `check Cargo.toml` evaluating direct dependencies at their currently resolvable versions needs range resolution with the standard library only (`x/mod/semver` has no ranges): in-house matchers under `internal/model/rangespec` for npm caret, tilde, x-ranges and `||`, PEP 440 specifier sets, and Cargo caret, tilde and exact requirements, resolved against the registry version list; `skipped: unsupported range` for anything else; table tests. Open question 10 decides whether this stays in M1 or moves to M2, where a sibling lockfile supplies versions and range resolution is the fallback | 8 h |
