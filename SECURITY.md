...
- CI runs `govulncheck`, `gosec` and `golangci-lint` on every push and pull request, plus a binary size gate, and runs the test suite under the race detector on Linux as well as without it on Linux, macOS and Windows.
...
- The signing identity is `release.yml@refs/tags/v*`, so whoever can push a `v*` tag can produce a release that verifies. Two repository settings narrow that. A tag ruleset lets only the repository's admin role create a `v*` tag, and the release job runs in a `release` environment whose required reviewer has to approve it before anything is built or signed, with admin bypass turned off. They are settings rather than files, so `docs/releasing.md` section 7 gives the commands that read them back; both were in place when this was last checked, on 2026-09-13.
