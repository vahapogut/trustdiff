...
- Reproducible builds: goreleaser with `-trimpath`, `CGO_ENABLED=0`, `-ldflags=-s -w`, `-buildvcs=false` and `SOURCE_DATE_EPOCH` taken from the commit, so a tag can be rebuilt and compared byte for byte.
...
2. Verify the archive against the signed checksum file.
   ...
   ```powershell
   (Get-FileHash .\trustdiff_0.0.1_windows_amd64.zip -Algorithm SHA256).Hash
...
3. Verify the build provenance with the GitHub CLI. This confirms the archive was built by this repository's release workflow from the tagged commit.
   ...
   ```sh
   gh attestation verify trustdiff_0.0.1_linux_amd64.tar.gz \
     --owner vahapogut \
