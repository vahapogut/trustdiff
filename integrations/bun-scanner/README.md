...
The setup behind that was done on 2026-09-10: the `trustdiff` organisation exists, `@trustdiff/bun-scanner@0.4.0` is public, and the trusted publisher is configured for this repository and this workflow file with staging permission. What follows is the record of those three steps, for anyone setting up the same thing elsewhere. `npm login` and then `sh scripts/publish-scanner.sh` from the repository root does the two that can be scripted and checks the third; [docs/releasing.md](../../docs/releasing.md) section 8 has them in full.
