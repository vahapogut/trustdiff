...
- **The binary is missing.** Every package of the install is unchecked, so it is reported the way any unchecked package is: one advisory at whatever `TRUSTDIFF_BUN_UNCHECKED` says, `warn` by default, naming the first package and saying how many it stands for. Bun asks about a warning on a terminal and cancels on it everywhere else, CI included. The line on stderr is still printed. Until v0.5.0 the scanner returned no advisories here, and Bun reads an empty list as every package being clean, so an install nothing was checked on went through exactly like one where nothing was wrong. Set `TRUSTDIFF_BUN_UNCHECKED=ignore` to get that back, or `TRUSTDIFF_BUN_REQUIRE_BINARY=1` in a repository that has decided every install must be scanned, where a missing binary stops the install outright with nothing to answer.
...
- **The report came back without a package that was asked about.** A document that leaves a package out is indistinguishable from one that found nothing wrong with it, so the missing ones are reported as unchecked at the `TRUSTDIFF_BUN_UNCHECKED` level, and `fatal` where any run exited 3. The trustdiff binary of this release answers about every ref it is given, so this finds nothing today; it costs a set of strings, and it is what keeps a future release or a replaced binary from turning silence into a pass.
...
No npm credential exists in this repository and none is meant to. The publishing itself is done by [`.github/workflows/npm-publish.yml`](../../.github/workflows/npm-publish.yml), through npm's trusted publishing: GitHub mints an OIDC token for that one workflow file, npm exchanges it for a short lived credential, and there is no secret to leak. It runs on a version tag rather than on the GitHub release, because a release created by the automatic `GITHUB_TOKEN` does not start another workflow run. It stages the version in `package.json` only on the tag that names exactly that version, `v` followed by it, and skips any other tag before it asks the registry about versions; it also skips a version the registry already has, asserts the tarball holds four files, and runs `bun test` before any of that.

It stages rather than publishes. A staged version sits on the registry where nobody can install it until a person approves it with `npm stage approve`, which is the one place this project can practise on itself what it argues for everywhere else. `npm stage list @trustdiff/bun-scanner` says what is waiting.

The setup behind that was done on 2026-09-10: the `trustdiff` organization exists, the package is published, and the trusted publisher is configured for this repository and this workflow file with staging permission. 0.4.0 went out by hand and 0.4.1 came through the workflow and an approval, so the chain has been exercised end to end. 0.5.0 was staged on 2026-09-11 and has not been made public; [docs/releasing.md](../../docs/releasing.md) section 9 has its stage id and what to check. What follows is the record of those three steps, for anyone setting up the same thing elsewhere. `npm login` and then `sh scripts/publish-scanner.sh` from the repository root does the two that can be scripted and checks the third; [docs/releasing.md](../../docs/releasing.md) section 9 has them in full.

1. **Create the npm organization.** The scope cannot be a personal one: npm gives every account only the scope matching its own name, so the account publishing this owns that scope and nothing else, and `@trustdiff` needs an organization literally named `trustdiff`. Organizations are created on npmjs.com only. Choose the free plan, which allows unlimited public packages, and turn on two-factor authentication, which `npm trust` requires. It has to be a passkey or a security key: npm no longer accepts an authenticator app for a new enrolment, verified 2026-09-10.
...
3. **Add the trusted publisher.** `npm trust github @trustdiff/bun-scanner --repo vahapogut/trustdiff --file npm-publish.yml --allow-stage-publish`, which needs npm 11.15.0 or newer and two-factor authentication, or the same thing through the package settings page. Neither the package name nor the permission flag is optional: without the name npm reads the wrong `package.json`, and without a flag it refuses, because there is no default permission. Then restrict token-based publishing on the package, which does not affect the trusted publisher because that uses OIDC rather than a token. The workflow's file name is part of the contract too: renaming it breaks publishing until the trusted publisher is reconfigured.

After that, bump `version` here to the version of the trustdiff release it belongs to, in that release's commit, add a line to the root `CHANGELOG.md`, and the release's tag stages it for approval. A release that leaves the scanner alone leaves the version here alone too, and its tag skips the scanner with `reason=tag-mismatch`.
