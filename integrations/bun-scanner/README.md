...
The setup behind that was done on 2026-09-10: the `trustdiff` organisation exists, the package is published, and the trusted publisher is configured for this repository and this workflow file with staging permission. 0.4.0 went out by hand and 0.4.1 came through the workflow and an approval, so the chain has been exercised end to end. What follows is the record of those three steps, for anyone setting up the same thing elsewhere. `npm login` and then `sh scripts/publish-scanner.sh` from the repository root does the two that can be scripted and checks the third; [docs/releasing.md](../../docs/releasing.md) section 8 has them in full.
...
After that, bump `version` here in the same commit as the trustdiff release it belongs to, add a line to the root `CHANGELOG.md`, and the tag stages it for approval.
