...
// userConfigPath returns the user-level fallback: <user config dir>/trustdiff/policy.yaml
// (XDG_CONFIG_HOME on Linux, Library/Application Support on macOS, AppData/Roaming on
...
func Find(startDir string) (path string, found bool, err error) {
	return find(startDir, os.UserConfigDir)
}

func find(startDir string, userConfigDir func() (string, error)) (string, bool, error) {
	...
	fallback, err := userConfigPath(userConfigDir)
	if err != nil {
