...
var checkRules = []checkRule{
	...
	{
		ID: "TD016", Name: "lockfile-entry-changed",
		Short: "A lockfile entry that changed without its version changing: another integrity hash, another source, or another resolved location.",
		Full: "A published release is immutable on every registry this tool reads, so one version under two " +
			"hashes means the file was written against bytes that were not that release. Four signals: " +
			"integrity-changed, integrity-removed, source-changed and resolved-changed. Only the two sides " +
			"of a diff can be compared, so an added entry and a scan report the check as skipped. An entry " +
			"that moved onto the registry, and a directory that moved to another directory, are reported at " +
			"info: the first is a project that stopped vendoring a dependency and the second is a workspace " +
